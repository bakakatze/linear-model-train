...
# Balance = four-point ordinal
# Vision = restricted and unrestricted
# 40 subjects: gender, height, weight, age

# Subjects were tested while standing on foam or a normal surface with their
# eyes open or closed. Each subject was tested twice for each surface and eye
# combinations for a total of 12 measurements per subject.

data(ctsib)
ctsib$stable = ifelse(ctsib$CTSIB == 1, 1, 0) # transform the result into completely stable (1 to 1) or not (2-4 to 0)
summary(ctsib)

# fit binomial GLM ignoring subject information entirely
gf = glm(stable ~ Sex + Age + Height + Weight + Surface + Vision, data = ctsib, family = binomial)
summary(gf)

# fixed subject factor
gfs = glm(stable ~ Sex + Age + Height + Weight + Surface + Vision + factor(Subject), data = ctsib, family = binomial)

anova(gf, gfs, test = "Chi")
# model with fixed subject factor is more significant

summary(gfs)
# but there are problems of identifiability and separability

# penalised quasi-likelihood method
gg = glmmPQL(stable ~ Sex + Age + Height + Weight + Surface + Vision,
             random = ~1|Subject,
             family = binomial,
             data = ctsib)
summary(gg)
...
# Let var Y_i ~=~ var(Y_i; beta, alpha)
# alpha represents parameters that model the correlation structure within individuals.
...
gg = gee(stable ~ Sex + Age + Height + Weight + Surface + Vision, 
         ...
         family = binomial,
         data = ctsib,
         corstr = "exchangeable",   # any pair of observations from the same subject have the same correlation
         scale.fix = TRUE)
# if we chose no correlation in this one, the GEE would be reduced to GLM.
# we chose a scale parameter at the default value of 1 to ensure maximum compatibolity with the GLMM fit (glmmPQL).

summary(gg)

## summary results:
# The estimates from GEE are about half the size of the GLMM (to be expected).
# Because: GLMM models the data at the subject level, the correlation between
# measurements on the individual is generated by the random effect. Thus, beta
# in the GLMM represents the effect on an individual.
# Whereas, a GEE models the data at the population level. The betas represent
